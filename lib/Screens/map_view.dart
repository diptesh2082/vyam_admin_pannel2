...
class _MapViewState extends State<MapView> {
  final controller = MapController(
      location: LatLng(22.572646, 88.363895)
  );
  ...
  void _onScaleEnd(ScaleEndDetails details) async{
    try{
      ...
      address = ((add.region ?? '')+'  '+(add.city ?? '')+'  '+(add.streetAddress ??'')+' '+(add.postal ??''));
    ...
    return Container(
      child: Stack(
        children: [
          ...
          const Align(
            alignment: Alignment.center,
            child: Icon(Icons.location_on_outlined, size: 38, color: Colors.yellowAccent,),
          ),
          Positioned(
            bottom: 10,
